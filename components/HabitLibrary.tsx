...
const ICON_OPTIONS: { icon: string; color: string; bg: string }[] = [
  ...
  { icon: "mci:hands-pray", color: "#6366F1", bg: "rgba(99, 102, 241, 0.1)" },
  ...
  { icon: "water-outline", color: "#06B6D4", bg: "rgba(6, 182, 212, 0.1)" },
  { icon: "mci:toothbrush", color: "#0EA5E9", bg: "rgba(14, 165, 233, 0.1)" },
  { icon: "people", color: "#A855F7", bg: "rgba(168, 85, 247, 0.1)" },
];

export function getHabitStyle(
  name: string,
  icon?: string,
...
export default function HabitLibrary({
  ...
  const itemWidth =
    (screenWidth - PARENT_PADDING - GAP * (COLUMNS - 1)) / COLUMNS;

  const normalizedExisting = existingHabitNames.map((n) => n.toLowerCase());

  const selectedStyle =
    ICON_OPTIONS.find((o) => o.icon === selectedIcon) ?? ICON_OPTIONS[0];
  ...
  return (
    <View>
        ...
        {PREDEFINED_HABITS.map((habit) => {
          ...
          return (
            <Pressable
              ...
              onPress={() =>
                !isAdded &&
                onAddHabit(habit.name, habit.icon, habit.defaultDuration)
              }
            ...
            >
              ...
              <Text
                ...
                style={{
                  color: isAdded ? Colors.mutedForeground : Colors.foreground,
                }}
                numberOfLines={1}
              >
                {habit.name}
      ...
      {/* Custom Input */}
      <View
        className="mt-4 rounded-2xl border-2 border-dashed border-border py-3 px-4"
        style={{ gap: 10 }}
      >
          ...
          <View className="flex-row" style={{ gap: 8 }}>
            {ICON_OPTIONS.map((opt) => {
              ...
              return (
                ...
                >
                  <HabitIcon name={opt.icon} size={20} color={opt.color} />
                </Pressable>
              );
            })}
          </View>
